...
void MultiMeterAudioVisualizer::paint(Graphics& g)
{
    ...
    {
        meterData.push_back(std::pair<float, std::string>(m_levelData.GetLevel(i).peak, std::to_string(i)));
    }
    ...
    for (const std::pair<float, std::string> &data : meterData)
    {
        ...
        g.fillRect(Rectangle<float>(meterLeft, visuAreaOrigY - meterHeight, meterWidth, meterHeight));
        g.drawText(data.second, Rectangle<float>(meterLeft, visuAreaOrigY, meterWidth, float(outerMargin)), Justification::centred, true);
